@tool
extends RefCounted
class_name CharacterCreationOptionsTask

signal completed(options: Dictionary, error: EiradirError)
...
func execute(request: HTTPRequest, baseUrl: String):
	...
	var endpoint: String = baseUrl + "/character_creation"
